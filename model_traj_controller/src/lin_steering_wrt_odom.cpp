...
void SteeringController::initializeSubscribers() {
    ...
    odom_subscriber_ = nh_.subscribe("/current_state", 1, &SteeringController::odomCallback, this); //subscribe to odom messages
...
void SteeringController::lin_steering_algorithm() {
    ...
    // Start with OL 
    // no feedback,i.e. no error assumption. Will see all 0 on debug outputs
